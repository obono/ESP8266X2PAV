...
#ifdef CUSTOMIZED
#include "common.h"
#define read(fd, buffer, length)    fd.read((uint8_t*)(buffer), length)
#define lseek(fd, offset, origin)   (fd.seek(offset, (SeekMode)(origin)), fd.position())
#define close(fd)                   fd.close()
#define fprintf(fd, ...)            dprintf(__VA_ARGS__)
#else
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
gd_open_gif(const char *fname)
{
    ...
    height = read_num(fd);
#ifdef CUSTOMIZED
    if (width > RENDER_SIZE_MAX || height > RENDER_SIZE_MAX) {
...
#ifdef CUSTOMIZED
        if (gif->loop_count > 0) gif->loop_count++;
#endif
        /* Skip block terminator. */
        lseek(gif->fd, 1, SEEK_CUR);
